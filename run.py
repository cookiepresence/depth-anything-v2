...
if __name__ == '__main__':
    ...
    parser.add_argument('--finetune-name', type=str, help='Name of finetuned model (required if not using pretrained)')
    parser.add_argument('--filenames-gt', type=str)
    ...
    if args.pretrained :
        ...
        depth_anything.load_state_dict(torch.load(f'saved_models/{args.finetune_name}'))
    ...
    filenames = [f"/scratch/soccernet/depth-basketball/Test/{args.img_path}"]
    filenames_gt = [f"/scratch/soccernet/depth-basketball/Test/{args.filenames_gt}"]
    
    os.makedirs(args.outdir, exist_ok=True)
    
    cmap = matplotlib.colormaps.get_cmap('Spectral_r')
    
    for k, (filename, filename_gt) in enumerate(zip(filenames, filenames_gt)):
